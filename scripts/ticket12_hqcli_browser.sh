...
if apt-get install -y yandex-browser-corporate >/dev/null 2>&1; then
    ...
    if command -v wget >/dev/null 2>&1; then
        wget -q -O /tmp/yandex-browser-corporate.rpm "$RPM_URL" 2>/dev/null || true
    elif command -v curl >/dev/null 2>&1; then
        curl -s -o /tmp/yandex-browser-corporate.rpm "$RPM_URL" 2>/dev/null || true
    fi
    if [[ -s /tmp/yandex-browser-corporate.rpm ]] && apt-get install -y /tmp/yandex-browser-corporate.rpm >/dev/null 2>&1; then
