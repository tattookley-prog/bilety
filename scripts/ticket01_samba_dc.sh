...
if [[ "$ROLE" == "1" ]]; then
    ...
    read -rp "Короткое имя этого сервера [br-srv]: " SRV_HOST; SRV_HOST="${SRV_HOST:-br-srv}"
    read -rp "DNS-форвардер [77.88.8.7]: " FWD; FWD="${FWD:-77.88.8.7}"

    echo
    info "Realm=$REALM  NetBIOS=$NBDOMAIN  IP=$SRV_IP  HOST=$SRV_HOST"
    ...
    # Исправление A-записи самого DC/его hostname, если provision подхватил docker0 (172.17.x.x)
    _ensure_dc_host_record_t01() {
        local rec="$1" target_ip="$2" current
        info "Проверяю A-запись контроллера домена: ${rec}.${DOMAIN_LC} → ${target_ip}"
        current="$(_dns_query_ip_t01 "$rec")"
        if [[ -z "$current" ]]; then
            warn "DNS: запись ${rec}.${DOMAIN_LC} отсутствует — добавляю"
            if _samba_dns_run_t01 dns add 127.0.0.1 "$DOMAIN_LC" "$rec" A "$target_ip" >/dev/null; then
                ok "DNS: ${rec}.${DOMAIN_LC} → ${target_ip} добавлена"
            else
                error "Не удалось добавить DNS-запись ${rec}.${DOMAIN_LC}"; return 1
            ...
            ok "DNS: ${rec}.${DOMAIN_LC} уже указывает на ${current} — ОК"
            return 0
        else
            warn "DNS: ${rec}.${DOMAIN_LC} указывает на ${current}, ожидается ${target_ip} — исправляю"
            if _samba_dns_run_t01 dns update 127.0.0.1 "$DOMAIN_LC" "$rec" A "$current" "$target_ip" >/dev/null; then
                ok "DNS: ${rec}.${DOMAIN_LC} обновлена: ${current} → ${target_ip}"
            else
                warn "update не сработал — удаляю и добавляю заново"
    ...
    echo; info "Исправление DNS-записи самого контроллера домена..."
    _ensure_dc_host_record_t01 "$SRV_HOST" "$SRV_IP" && STATUS[dns_dc_host]=OK || STATUS[dns_dc_host]=ERROR
...
EOF
    ...
    if [[ -f "$_SSSD_CONF_MKHOME" ]]; then
        ...
        _add_sssd_domain_option fallback_homedir '/home/%u' "$_SSSD_CONF_MKHOME"
        _add_sssd_domain_option default_shell '/bin/bash' "$_SSSD_CONF_MKHOME"
        systemctl restart sssd 2>/dev/null || true
        sleep 2
    else
...
samba-tool dns query 127.0.0.1 au-team.irpo br-srv A -U administrator # A-запись DC hostname
host au-team.irpo                                     # Проверка резолвинга домена
host br-srv.au-team.irpo                              # Проверка резолвинга имени DC
