...
| `E: Невозможно заблокировать /var/cache/apt/archives/lock` | Блокировку apt держит другой процесс (packagekit/apt-indicator) или зависший `apt-get` от прерванного предыдущего запуска | Скрипт автоматически останавливает `packagekit` перед ожиданием apt lock. Если lock не освободился, выполните `fuser -v /var/cache/apt/archives/lock /var/lib/apt/lists/lock`; `pkill -f apt-get` используйте вручную только если `fuser` показывает зависший/устаревший процесс. Удалять сам файл lock (`rm -f`) только если `fuser` не показал держащих его процессов; затем перезапустить скрипт |
...
## Продажа скриптов учебной группе (лицензионная система)

Если ты хочешь **продавать** скрипты своей учебной группе, в репозитории реализована серверная система лицензирования:

- **20 логинов × 5 запусков каждый** — выдаются только оплатившим покупателям.
- Сами скрипты хранятся **только на твоём сервере** и никогда не попадают в публичный репозиторий.
- Покупатель использует `client/client.sh` — тонкий загрузчик, который запрашивает скрипт у сервера после проверки учётных данных.

### Быстрый старт

```bash
# Покупатель запускает:
export BILETY_SERVER_URL="https://<твой-логин>.pythonanywhere.com"
bash client/client.sh
```

### Подробная инструкция по развёртыванию

Полное руководство (на русском) — в файле **[docs/SELLING_SETUP.md](docs/SELLING_SETUP.md)**:

- Архитектура системы (публичный клиент → твой сервер → приватные скрипты)
- Развёртывание Flask-сервера на **PythonAnywhere** (бесплатный тариф)
- Настройка базы данных и генерация 20 логинов
- Выдача доступа после оплаты через `admin.py`
- Инструкция для покупателей
- Перенос на Fly.io / VPS
